...
async function setBulbColour(req, res, next) {
    ...
    try {
        ...
        if (!bulb.supportsColor) return res.status(400).json({
            status: 'error',
...
async function setColourTemperature(req, res, next) {
    ...
    try {
        ...
        if (!bulb.supportsColorTemperature) return res.status(400).json({
            status: 'error',
            message: 'Your bulb does not support color'
        });
        ...
        if ((temperature < range.min || temperature > range.max) && temperature !== 0) return res.status(400).json({
            status: 'error',
