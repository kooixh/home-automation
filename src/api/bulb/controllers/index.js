...
async function setBulbBrightness(req, res, next) {
  ...
  if (!req.body || !req.body.hasOwnProperty('brightness')) {
    return next(new Error('Body does not have brightness field.'))
  }

  if (isNaN(req.body.brightness) || req.body.brightness === '') {
    return next(new Error('Brightness value is not a number'))
  }
  let brightness = parseInt(req.body.brightness)

  if (brightness < 0 || brightness > 100) {
    return next(new Error('Invalid brightness level'))
  }
  ...
  try {
    ...
    if (bulb === null) {
      return res.status(400).json({ status: 'error', message: 'Device not found for host' })
    }
...
async function setBulbColour(req, res, next) {
  ...
  if (!req.body || !req.body.hasOwnProperty('color')) {
    return next(new Error('Body does not have colour field.'))
  }

  if (!colourUtils.isHEXColour(req.body.color)) {
    return next(new Error('Colour has to be in hex value. e.g: 2f458a'))
  }
  ...
  try {
    ...
    if (!bulb.supportsColor) {
      return res.status(400).json({
        status: 'error',
        message: 'Your bulb does not support color',
      })
    }
...
async function setColourTemperature(req, res, next) {
  ...
  if (!req.body || !req.body.hasOwnProperty('temperature')) {
    return next(new Error('Body does not have temperature field.'))
  }

  if (isNaN(req.body.temperature) || req.body.temperature === '') {
    return next(new Error('Brightness value is not a number'))
  }
  ...
  try {
    ...
    if ((temperature < range.min || temperature > range.max) && temperature !== 0) {
      return res.status(400).json({
        status: 'error',
        message: 'Color temperature value out of range',
      })
    }
