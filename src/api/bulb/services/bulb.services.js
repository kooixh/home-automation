...
async function getDeviceByIP(host) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            return (await client.getTypeFromSysInfo(bulb._sysInfo) === 'bulb') ? resolve(bulb) :
...
async function turnBulbOn(bulb) {
    ...
    return {
        message: bulb.alias + ' switched on',
        status: 'success'
    };
}
...
async function turnBulbOff(bulb) {
    ...
    return {
        message: bulb.alias + ' switched off',
        status: 'success'
    };
}
...
async function setBulbBrightness(bulb, brightness) {
    ...
    return {
        message: bulb.alias + ' brightness set to ' + brightness,
        status: 'success'
    };
}
...
async function setBulbColour(bulb, hsv) {
    ...
    let temperature = (hue === 0 && saturation === 0) ? WHITE_COLOUR_TEMP : 0;
    await bulb.lighting.setLightState({hue: hue, saturation: saturation, brightness: value, color_temp: temperature});
    return {
        message: bulb.alias + ' colour set to HSV ' + hue + ' ' + saturation + ' ' + value,
        status: 'success'
    };
}
...
async function setBulbTemperature(bulb, temperature) {
    ...
    return {
        message: bulb.alias + ' colour temperature set to ' + temperature,
        status: 'success'
    };
}
