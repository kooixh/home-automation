...
async function saveInDatabase(deviceList) {
  return new Promise(async (resolve, reject) => {
    let devicesInfo = await Promise.all(
      _.map(deviceList, async (elem) => {
...
function updateDevice(collection, elem) {
  collection.findOneAndUpdate({ deviceId: elem.deviceId }, { $set: elem }, { upsert: true }, (err, r) => {
    if (err) {
      return reject(err)
    }
    console.info(r)
...
async function getDevice(host) {
  try {
    ...
    device = JSON.parse(
      JSON.stringify(device, function (key, value) {
